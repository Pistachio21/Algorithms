class Grid {
    ...
    checkSurroundingCells(row: number, col: number) {
        ...
        directions.forEach(([dx, dy]) => {
            ...
            if (newRow >= 0 && newRow < this.size && newCol >= 0 && newCol < this.size && this.grid[newRow][newCol] === 2) {
                this.grid[row][col] = 2;
            } else if (newRow >= 0 && newRow < this.size && newCol >= 0 && newCol < this.size && this.grid[newRow][newCol] === 1) {
                this.grid[newRow][newCol] = 2;
            }
        });
    }
    
    topAndBottomAreConnected(): boolean {
        for (let col = 0; col < this.size; col++) {
            if (this.grid[0][col] === 2 && this.grid[this.size - 1][col] === 2) {
                return true;
            }
        }
        return false;
    ...
    startRandomOpensiteEverySecond(): void {
        this.intervalId = setInterval(() => {
            ...
            if (this.topAndBottomAreConnected()) {
                if (this.intervalId) {
